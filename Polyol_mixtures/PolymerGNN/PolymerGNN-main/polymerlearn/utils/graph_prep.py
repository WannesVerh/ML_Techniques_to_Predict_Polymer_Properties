...
class GraphDataset:
    ...
    def __init__(self,
        ...
        #ac indicates the columns of the alcohols (20-33)
        #gc indicates the columns of the glycols (34-46)
        if type(ac) == tuple:
            self.ac_tuple = True
        else:
            self.ac_tuple = False
        ...
        for i in range(data.shape[0]): 
            ...
            if self.ac_tuple: #if tuple
                acid_hit = (data.iloc[i,ac[0]:ac[1]].to_numpy() > 0)  #go throuhg all the colums and return true if acid conc >0
            else:
                acid_hit = (data[ac].iloc[i].to_numpy() > 0)
            if self.gc_tuple:
                glycol_hit = (data.iloc[i,gc[0]:gc[1]].to_numpy() > 0)
            ...
            # Add to percentage lists:
            if self.ac_tuple:
                acid_pcts.append(data.iloc[i,ac[0]:ac[1]][acid_hit].tolist()) #only ad those that have a hit(so where the array is true)
            else:
                acid_pcts.append(data[ac].iloc[i][acid_hit].tolist())
            if self.gc_tuple:
                glycol_pcts.append(data.iloc[i,gc[0]:gc[1]][glycol_hit].tolist())
            ...
            acid_included.append(acid_pos) #gives the names of the acids used matrix with every row a certain polymer 
            glycol_included.append(glycol_pos)
        ...
        if self.z_pos_loaders: # Load the Z-pos structure (off by default)

            for i in range(len(acid_included)): #for every polymer used
                A_sub = []
                for j in range(len(acid_included[i])): #for every acid used per polymer
                    #read_Xyz_files and returns atoms, charge, xyz_coordinates
            ...
            for i in range(len(acid_included)): #for every polymer used
                #make RDKit objects of all the acids present
                #!
        ...
        # Set Y (target)
        Y = data.loc[:,Y_target] #selected the colum with the y-values
        ...
        non_nan_mask = Y.notna() #true if value, false if no value

        self.exclude_inds = exclude_inds #default is none
        ...
        if self.exclude_inds is not None:
            ...
            exclude_by_index = [True] * Y.shape[0]  #make an array with all true

        self.bound_filter = bound_filter #default none
        
        if self.bound_filter is not None:
            non_nan_mask = non_nan_mask & (Y > bound_filter[0]) & (Y < bound_filter[1]) & exclude_by_index
        
        non_nan_mask['res_bool'] = False #make a row under the True false values values, filled with false 
        non_nan_mask.loc[non_nan_mask[Y_target].all(1), 'res_bool'] = True #if there is a false value make res_bool true
    ...
    def get_train_batch(self, size: int):
        ...
        sample_inds = random.sample(list(np.arange(len(self.Ytrain))), k = size) #randopmly pick indicis
